...
5. `prepare_task_context`：接到自然语言开发任务时优先使用，输出 AI 可直接行动的上下文包。
6. `explain_feature_for_agent`：已经知道 feature key 时使用，输出功能记忆卡片。
7. `analyze_change_impact`：改完或 review 前使用，按 changed files / diff 判断影响面和验证建议。
8. `query_project_chain` / `query_feature_chain`：需要进一步追某条 selector 链路时使用；默认 `freshnessPolicy=auto_rebuild`，工具会在 KB 过期、缺失或未知时同步重建并等待 `fresh` 后再返回结果。
9. `build_project_index` / `build_feature_index`：只在需要手动维护、预热或排查构建问题时直接调用。手动预热 project-global 时优先用 `start_build_project_index` 并传 `wait:true`。
10. `discover_features` + `build_feature_index`：需要功能级链路、入口拆分或多模块边界时生成功能 KB。
...
- 自然语言开发任务：先用 `prepare_task_context`，例如 task=`修改 settings 页 AI 配置保存逻辑`。
...
- 使用 Agent Context Pack 时，把 `evidence` 中的 `file`、`method`、`endpoint`、`nodeId` / `edgeType` 和 `confidence` 作为计划、编辑边界和 review 的依据。
