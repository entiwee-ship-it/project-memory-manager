...
6. `build_project_index` / `build_feature_index`：只在需要手动维护、预热或排查构建问题时直接调用。
7. `discover_features` + `build_feature_index`：需要功能级链路、入口拆分或多模块边界时生成功能 KB。

CLI 只作为兜底或维护入口。MCP 不可用时再使用 `src/bin/*.js` 命令。

如果使用 `start_build_project_index` 异步构建，必须轮询 `get_job_status` 到 `succeeded`，再查询。不能启动异步构建后直接用旧 KB 或直接回源码大范围搜索。
