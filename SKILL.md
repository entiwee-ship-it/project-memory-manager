...
5. `prepare_agent_brief`：接到开发任务时优先使用，聚合 Usage Gate、执行计划、历史任务记忆、项目 playbook、推荐文件和验证命令。
6. `decide_pmm_usage`：只需要判断是否必须深度 PMM 时使用。
7. `plan_task_execution`：需要单独生成执行计划时使用，输出 PMM gate、目标文件、编辑边界、步骤、验证命令和证据。
8. `recall_task_memory`：需要单独召回历史任务 outcome、相关文件、验证命令和观察时使用。
9. `prepare_task_context`：需要更细任务上下文时使用，输出 AI 可直接行动的上下文包。
10. `explain_feature_for_agent`：已经知道 feature key 时使用，输出功能记忆卡片。
11. `analyze_change_impact`：改完或 review 前使用，按 changed files / diff 判断影响面和验证建议。
12. `validate_edit_scope`：提交前使用，复核 changed files 是否越过 PMM 建议边界、是否含高风险文件、是否疑似漏改关键文件。
13. `review_patch_for_agent`：AI 自检或 code review 前使用，生成 verdict、findings 和复核清单。
14. `record_task_outcome`：任务结束后记录结果、改动文件、验证命令和观察，供后续会话参考。
15. `update_project_playbook`：阶段性沉淀稳定项目规则，或从 task/outcome/changedFiles 中推断项目惯例。
16. `summarize_project_memory`：跨会话接力或复盘时查看当前项目已沉淀的记忆。
17. `query_project_chain` / `query_feature_chain`：需要进一步追某条 selector 链路时使用；默认 `freshnessPolicy=auto_rebuild`，工具会在 KB 过期、缺失或未知时同步重建并等待 `fresh` 后再返回结果。
18. `build_project_index` / `build_feature_index`：只在需要手动维护、预热或排查构建问题时直接调用。手动预热 project-global 时优先用 `start_build_project_index` 并传 `wait:true`。
19. `discover_features` + `build_feature_index`：需要功能级链路、入口拆分或多模块边界时生成功能 KB。
