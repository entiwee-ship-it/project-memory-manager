...
- 查询调用链、事件、request、state 时，优先运行 `scripts/query_chain_kb.js --feature ...`，不要把全仓库搜索作为第一轮动作
...
- 可在目标仓库任意子目录运行 `node <skill-path>/scripts/query_chain_kb.js --feature <feature-key> ...`
