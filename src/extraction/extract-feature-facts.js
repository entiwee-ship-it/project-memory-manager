...
const NEXT_ROUTE_METHODS = new Set(['GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'HEAD', 'OPTIONS']);
...
const PRISMA_READ_METHODS = new Set(['findUnique', 'findFirst', 'findMany', 'count', 'aggregate', 'groupBy']);
...
function extractBindingNames(nameNode, acc = [], ts = TYPESCRIPT_RUNTIME) {
    if (!ts || !nameNode) {
        return acc;
    }
    ...
    if (ts.isObjectBindingPattern(nameNode) || ts.isArrayBindingPattern(nameNode)) {
        for (const element of nameNode.elements || []) {
            ...
            extractBindingNames(element.name, acc, ts);
        }
    }
    return acc;
}
...
function extractAstParamNames(params = [], ts = TYPESCRIPT_RUNTIME) {
    ...
    return uniqueSorted(
        params.flatMap(param => extractBindingNames(param.name, [], ts))
    );
}

function extractMethodBodyText(bodyNode, sourceFile, ts = TYPESCRIPT_RUNTIME) {
    if (!ts || !bodyNode) {
        return '';
    }
...
function extractDirectBodyText(functionNode, sourceFile, ts = TYPESCRIPT_RUNTIME) {
    if (!ts || !functionNode?.body) {
        return '';
    }

    const bodyNode = functionNode.body;
    const fullBodyText = extractMethodBodyText(bodyNode, sourceFile, ts);
    if (!fullBodyText) {
        return fullBodyText;
    }
...
function inferFieldType(member, sourceFile, ts = TYPESCRIPT_RUNTIME) {
    if (!ts || !member) {
        return '';
    }
...
function extractAstFieldTypes(source, scriptFile, imports) {
    const ts = TYPESCRIPT_RUNTIME;
    if (!ts) {
        return new Map();
    }
    ...
    const visit = node => {
        if (ts.isClassLike(node)) {
            for (const member of node.members || []) {
                ...
                const decoratorType = inferFieldType(member, sourceFile, ts);
...
function extractFieldTypes(source, scriptFile, imports) {
    const fieldTypes = extractAstFieldTypes(source, scriptFile, imports);
    const importMap = new Map();
...
function normalizeInlineExpression(expression) {
    ...
    const normalized = stripCodeComments(expression)
        .replace(/\s+/g, ' ')
        .trim();
...
function normalizeHttpTarget(expression, options = {}) {
    let target = normalizeInlineExpression(expression || '');
    if (!target) {
        return '';
    }
    target = target
        .replace(/\$\{\s*API_BASE\s*\}/g, '/api')
        .replace(/\bAPI_BASE\b/g, '/api')
        .replace(/\$\{[^}]+\}/g, '')
        .replace(/[?&]$/, '');
    ...
    if (options.assumeApiBase && target.startsWith('/') && !target.startsWith('/api')) {
        target = `/api${target}`;
    }
...
function isDynamicHttpTarget(target) {
    ...
    if (value.includes('${') || /\bendpoint\b/.test(value)) {
        return true;
    }
...
function isDynamicHttpExpression(expression, normalizedTarget = '') {
    ...
    if (/\bendpoint\b/.test(raw) && (!target || target === '/api')) {
        return true;
    }
    if (raw.includes('${') && target === '/api') {
        return true;
    }
    return false;
}
...
function isExternalImportSpecifier(specifier = '') {
    ...
    if (value.startsWith('node:') || value.includes(':')) {
        return false;
    }
    return true;
}
...
function buildExternalService(name, options = {}) {
    const normalizedName = String(name || '').trim();
    if (!normalizedName) {
        return null;
    }
    const target = normalizeHttpTarget(options.target || '');
    return {
        name: normalizedName,
        serviceKind: options.serviceKind || 'external-service',
        target,
        host: options.host || hostFromUrl(target),
        packageName: options.packageName || '',
        source: options.source || '',
    };
}

function inferExternalService(target, source = 'http') {
    const normalizedTarget = normalizeHttpTarget(target || '');
    const text = normalizedTarget.toLowerCase();
    if (!text) {
        return null;
    }
    if (text.includes('graph.facebook.com') || text.includes('facebook.com')) {
        return buildExternalService('Facebook Graph API', {
            serviceKind: 'http-api',
            target: normalizedTarget,
            source,
        });
    }
    if (text.includes('anthropic.com') || text.includes('claude')) {
        return buildExternalService('Anthropic Claude', {
            serviceKind: 'ai-api',
            target: normalizedTarget,
            source,
        });
    ...
    if (/^https?:\/\//i.test(normalizedTarget)) {
        const host = hostFromUrl(normalizedTarget);
        return buildExternalService(host || normalizedTarget, {
            serviceKind: 'http-api',
            target: normalizedTarget,
            host,
            source,
...
function inferImportedService(importInfo, methodBody) {
    ...
    if (specifier === '@anthropic-ai/sdk' && /\b(?:new\s+Anthropic|Anthropic|messages\.create)\b/.test(body)) {
        return buildExternalService('Anthropic Claude', {
            serviceKind: 'ai-sdk',
            packageName,
            source: 'import',
    ...
    if (specifier === '@prisma/client' || /\bPrismaClient\b/.test(body)) {
        return buildExternalService('Prisma ORM', {
            serviceKind: 'orm',
            packageName,
            source: 'import',
        });
    }
    if (specifier === 'next/server' || specifier.startsWith('next/')) {
        return buildExternalService('Next.js Runtime', {
            serviceKind: 'framework',
            packageName: 'next',
            source: 'import',
        });
    }
    if (specifier === 'next-auth' || specifier.startsWith('next-auth/') || specifier.startsWith('@auth/')) {
        return buildExternalService('NextAuth', {
            serviceKind: 'auth',
            packageName,
            source: 'import',
        });
...
function extractExternalServices(methodBody, imports = [], networkRequests = []) {
    const services = [];
    ...
    if (/\bFACEBOOK_GRAPH_API_BASE\b|graph\.facebook\.com|www\.facebook\.com\/v\d+\.\d+/i.test(body)) {
        pushService(buildExternalService('Facebook Graph API', {
            serviceKind: 'http-api',
            target: 'https://graph.facebook.com',
            source: 'literal',
        }));
    }
    if (/\bprisma\.[A-Za-z_$][\w$]*\.|\btx\.[A-Za-z_$][\w$]*\./.test(body)) {
        pushService(buildExternalService('Prisma ORM', {
            serviceKind: 'orm',
            source: 'prisma-call',
        }));
    }
    ...
    return dedupeBy(
        services,
        service => `${service.name || ''}::${service.serviceKind || ''}::${service.target || ''}::${service.packageName || ''}`
    );
}
 ...
 * - 限制长度避免输出过大
 */
function buildBodySnippet(bodyText, maxLength = 500) {
    if (!bodyText) {
        return '';
    }
...
function inferPinusRoutes(scriptFile, methodName) {
    const normalizedPath = normalize(scriptFile);
...
function extractTableBindings(source) {
    const bindings = [];
    const ts = TYPESCRIPT_RUNTIME;
    if (!ts) {
        return bindings;
    ...
    const visit = node => {
            ...
            for (const member of node.members || []) {
                if (ts.isMethodDeclaration(member)) {
                    ...
                    pushDefinition(name, member, {
                        ...
                        bodyText: extractMethodBodyText(member.body, sourceFile, ts),
                        directBodyText: extractDirectBodyText(member, sourceFile, ts),
                        astKind: 'method',
                        ownerName,
                        ownerBasePath,
                    });
                ...
                if (
                    ...
                    pushDefinition(name, member, {
                        ...
                        bodyText: extractMethodBodyText(initializer.body, sourceFile, ts),
                        directBodyText: extractDirectBodyText(initializer, sourceFile, ts),
        ...
        if (ts.isFunctionDeclaration(node) && node.name && node.body) {
            ...
            pushDefinition(name, node, {
                ...
                bodyText: extractMethodBodyText(node.body, sourceFile, ts),
                directBodyText: extractDirectBodyText(node, sourceFile, ts),
                astKind: 'function',
            });
        }
                    ...
                    for (const property of declaration.initializer.properties || []) {
                        if (ts.isMethodDeclaration(property)) {
                            ...
                            pushDefinition(propertyName, property, {
                                ...
                                bodyText: extractMethodBodyText(property.body, sourceFile, ts),
                                directBodyText: extractDirectBodyText(property, sourceFile, ts),
...
function analyzeInlineCallable(callbackNode, imports, fieldTypes, handlerMaps, parentParamNames = [], methodName = '__inline__', knownMethodNames = []) {
    const ts = TYPESCRIPT_RUNTIME;
    if (!ts || !callbackNode) {
    ...
    const bodyText = extractMethodBodyText(callbackNode.body, sourceFile, ts);
    if (!bodyText) {
        return createEmptyCallInfo();
    }
    ...
    const astInfo = extractAstMethodCalls(
        {
            name: methodName,
...
function extractAstMethodCalls(methodDef, imports, fieldTypes, handlerMaps, paramNames = [], knownMethodNames = []) {
    const ts = TYPESCRIPT_RUNTIME;
        ...
        if (ts.isCallExpression(node)) {
            ...
            if (pinusRpcMatch) {
                ...
                const callbackCallInfo = callbackArg
                    ? analyzeInlineCallable(callbackArg, imports, fieldTypes, handlerMaps, paramNames, '__pinus_rpc_callback__', knownMethodNames)
                    : createEmptyCallInfo();
            ...
            if (['eventBus.on', 'eventBus.once', 'oops.message.on', 'oops.message.once', 'director.on', 'director.once'].includes(normalizedPath)) {
                ...
                if (eventName && handlerPath) {
                    ...
                    const inlineCallInfo = analyzeInlineCallable(handlerArg, imports, fieldTypes, handlerMaps, paramNames, '__inline_event__', knownMethodNames);
                    eventSubscriptions.push({
                        bus: normalizedPath.split('.').slice(0, -1).join('.'),
                        mode: normalizedPath.split('.').slice(-1)[0],
                        event: eventName,
            ...
            if (normalizedPath === 'VM.bindPath') {
                ...
                if (eventName && handlerPath) {
                    ...
                    const inlineCallInfo = analyzeInlineCallable(handlerArg, imports, fieldTypes, handlerMaps, paramNames, '__inline_vm_event__', knownMethodNames);
                    eventSubscriptions.push({
                        bus: 'VM',
                        mode: 'bindPath',
                        event: eventName,
            ...
            if (['Net.inst.request', 'Net.inst.tableMsg'].includes(normalizedPath)) {
                ...
                const callbackMethodRef = stripThisPrefix(readThisAccessPathFromAst(lastArg, ts) || '');
                const callbackCallInfo = callbackArg
                    ? analyzeInlineCallable(callbackArg, imports, fieldTypes, handlerMaps, paramNames, '__network_callback__', knownMethodNames)
                    : createEmptyCallInfo();
            ...
            if (['setTimeout', 'setInterval', 'scheduleOnce'].includes(normalizedPath)) {
                ...
                const callbackCallInfo = callbackArg && (ts.isArrowFunction(callbackArg) || ts.isFunctionExpression(callbackArg))
                    ? analyzeInlineCallable(callbackArg, imports, fieldTypes, handlerMaps, paramNames, '__timing_callback__', knownMethodNames)
                    : createEmptyCallInfo();
...
function summarizeInlineAction(callInfo) {
    return {
        ...
        externalServices: callInfo.externalServices,
    };
}
...
function extractDbAccesses(methodBody, imports = []) {
    ...
    const prismaPattern = /\b(?:prisma|tx)\.([A-Za-z_$][\w$]*)\.(findUnique|findFirst|findMany|count|aggregate|groupBy|create|createMany|update|updateMany|upsert|delete|deleteMany)\s*\(/g;
    while ((match = prismaPattern.exec(methodBody))) {
        ...
        if (PRISMA_READ_METHODS.has(operation)) {
            reads.push(access);
        } else if (PRISMA_WRITE_METHODS.has(operation)) {
            writes.push(access);
        }
    }
...
function normalizeRouteSegment(segment) {
    ...
    const catchAll = value.match(/^\[\.\.\.([A-Za-z_$][\w$-]*)\]$/);
    if (catchAll) {
        return `:${catchAll[1]}*`;
    }
    const dynamic = value.match(/^\[([A-Za-z_$][\w$-]*)\]$/);
    if (dynamic) {
        return `:${dynamic[1]}`;
    }
    return value;
...
function buildRouteEndpointPath(scriptFile) {
    const normalizedScriptFile = normalize(scriptFile);
    const match = normalizedScriptFile.match(/(?:^|\/)app\/api\/(.+)\/route\.(?:ts|tsx|js|jsx)$/i);
    if (!match) {
        return '';
    }
...
function attachRouteEndpoints(methods, scriptFile) {
    const endpointPath = buildRouteEndpointPath(scriptFile);
    if (!endpointPath) {
        return;
    }
    for (const method of methods) {
        const routeMethod = String(method.name || '').trim().toUpperCase();
        if (!NEXT_ROUTE_METHODS.has(routeMethod)) {
            continue;
        }
        method.httpEndpoints = Array.isArray(method.httpEndpoints) ? method.httpEndpoints : [];
        method.httpEndpoints.push({
            method: routeMethod,
            path: endpointPath,
            handlerName: method.ownerName ? `${method.ownerName}.${method.name}` : method.name,
            handlerIdentifier: '',
            handlerMethod: method.name,
            handlerSourcePath: normalize(scriptFile),
            handlerSourceSpecifier: '',
            middlewares: [],
            localCalls: method.localCalls || [],
            importedCalls: method.importedCalls || [],
            fieldCalls: method.fieldCalls || [],
            sourceKind: 'next-app-route',
        });
        method.summary = method.summary || `Next.js ${routeMethod} ${endpointPath}`;
    }
}

function attachMessageRoutes(methods, scriptFile, source) {
    const tableBindings = extractTableBindings(source);
    ...
    for (const method of methods) {
        ...
        method.externalServices = Array.isArray(method.externalServices) ? method.externalServices : [];

        method.messageRoutes.push(...inferPinusRoutes(scriptFile, method.name));
...
function buildHttpRequest(callee, target, httpMethod, transport) {
    ...
    return {
        callee,
        target: normalizedTarget || callee,
        protocol: 'http',
...
function extractNetworkRequests(methodBody, imports, fieldTypes, handlerMaps, paramNames, knownMethodNames = []) {
    ...
    const fetchJsonPattern = /\bfetchJSON(?:<[\s\S]*?>)?\s*\(/g;
    while ((match = fetchJsonPattern.exec(methodBody))) {
        ...
        fetchJsonPattern.lastIndex = openParenIndex + content.length + 2;
    }

    const eventSourcePattern = /\bnew\s+EventSource\s*\(/g;
    while ((match = eventSourcePattern.exec(methodBody))) {
        const openParenIndex = eventSourcePattern.lastIndex - 1;
        const content = extractWrappedContent(methodBody, openParenIndex, '(', ')');
        const args = splitTopLevelArgs(content);
...
function extractMethodCalls(methodBody, methodName, imports, fieldTypes, handlerMaps, paramNames = [], knownMethodNames = [], directMethodBody = '') {
    ...
    const importedCalls = [];
    const importScanBody = methodBody || directBody;
    for (const importInfo of imports) {
        for (const identifier of importInfo.identifiers) {
            const escapedIdentifier = identifier.replace(/\$/g, '\\$');
            const directFunctionPattern = new RegExp(`\\b${escapedIdentifier}(?:<[^;()]*>)?\\s*\\(`, 'g');
            let directFunctionMatch = null;
            while ((directFunctionMatch = directFunctionPattern.exec(importScanBody))) {
                const previousChar = importScanBody[Math.max(0, directFunctionMatch.index - 1)] || '';
                if (previousChar === '.') {
                    continue;
                }
            ...
            const callPattern = new RegExp(`\\b(${escapedIdentifier}(?:\\.[A-Za-z_$][\\w$]*)*)\\.([A-Za-z_$][\\w$]*)(?:<[^;()]*>)?\\s*\\(`, 'g');
            let callMatch = null;
            while ((callMatch = callPattern.exec(importScanBody))) {
...
function mergeCallInfo(regexInfo, astInfo) {
    ...
    const mergedExternalServices = dedupeBy(
...
function normalizeFinalCallInfo(callInfo, knownMethodNames = []) {
    const normalized = {
        ...
        externalServices: dedupeBy(
            [...(callInfo.externalServices || [])],
            item => `${item.name || ''}::${item.serviceKind || ''}::${item.target || ''}::${item.packageName || ''}`
        ),
    };
    ...
    for (const root of methodRoots) {
        ...
        for (const scriptFile of scriptFiles) {
            ...
            for (const methodDef of methodDefinitions) {
                ...
                const astCallInfo = extractAstMethodCalls(methodDef, imports, fieldTypes, handlerMaps, paramNames, knownMethodNames);
                ...
                const bodySnippet = buildBodySnippet(methodBody, maxLength);
