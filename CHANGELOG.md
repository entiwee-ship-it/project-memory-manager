...
### 文档
- `docs/user/install-from-github.md` 新增「8.1 多 Agent 副本拓扑」小节，覆盖完整安装与精简安装两种形态、6 个安装位置、手工同步全部副本的示例脚本。
- 新增「8.2 `skills` CLI 已知行为」小节，记录三个已知坑：报失败但部分成功（需逐个核对版本号）、合并式更新（不删除上游已移除的文件）、子集安装（不能用整仓克隆覆盖）。
- 故障排查小节新增 `skills update` 后副本版本不一致的排查路径。
- 升级小节补充推荐手工同步的说明。

## [0.85.4] - 2026-09-12

### 改进
- 锁的过期判定改为优先依据持有进程的存活状态，时间戳超时降级为兜底。新增 `isProcessAlive` 与 `inspectLock` 的存活分支：持有进程已退出（`ESRCH`）时锁立即回收，持有进程仍存活时锁继续有效。这同时修掉了旧判据的两个相反方向的失效——持有进程已退出的残留锁会一直阻塞到超时为止（重建锁为 30 分钟），而长时间运行的操作会在声明超时后被并发进程抢锁。
- 无法判断持有进程存活（pid 缺失或非法）时仍退回时间戳判定，保持不会永久阻塞。
- 被锁阻塞时的诊断信息新增「持有进程」段落，给出 pid 和起始时间，便于区分正常并发与进程卡死。

### 测试
- `tests/lock-diagnostics.test.js` 新增存活探测、已退出持有者立即回收、存活持有者不被超时抢占、无法判断时退回超时、无效时间戳且无法判断持有者共 5 个用例。
- `tests/rebuild-lock.test.js` 移除基于固定超时的旧行为断言，保留超时余量与锁文件记录断言，避免与新的存活判定语义冲突。
...
### 修复
- `message` selector 未命中时返回中文语义说明，明确它只查协议/事件消息名，不是自然语言问题入口。
- 文档和技能说明补充自然语言问题的正确查询方式：先抽取 `endpoint`、`request`、`method` 或关键词，再使用对应 selector。
